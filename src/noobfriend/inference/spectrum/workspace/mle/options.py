...
def build_mle_options(
    *,
    ...
    if not 0.0 < likelihood <= 1.0:
        raise ValueError(
            "relative_likelihood_min must be greater than 0 and at most 1."
        )
...
def _normalize_multipliers(values: Sequence[float]) -> tuple[float, ...]:
    if isinstance(values, (str, bytes)):
        raise TypeError(
            "absorption_bound_multipliers must be a sequence of positive numbers."
        )
    try:
    ...
    parsed = tuple(
        _finite_float(value, name="absorption_bound_multipliers") for value in iterator
    )
    if not parsed:
        raise ValueError("absorption_bound_multipliers must not be empty.")
    if any(value <= 0.0 for value in parsed):
        raise ValueError(
            "absorption_bound_multipliers must contain only positive values."
        )
