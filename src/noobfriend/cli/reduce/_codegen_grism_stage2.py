...
Pass 1 reduces each dispersed frame rate -> 2b (assign_wcs -> flat -> WFSS
master-sky -> flag_outlier -> column 1/f) and collects a downsampled, trace-masked
residual grid per (module, dispersion direction). Between passes a sky-residual
template is built for each group; pass 2 scalar-fits and subtracts it (2b ->
2bii). The template is a sidecar, not a NooBox record; 2bii -> 2b lineage is
stamped.
...
#: Bespoke column-only, trace-masked 1/f. GRISMR/GRISMC traces run along a
#: detector axis, so only the cross-dispersion (column) axis is destriped, with
#: the dispersed traces masked out of the per-column medians. Additive readout
#: correction that follows the sky subtraction.
_ONEOVERF: tuple[str, ...] = (
    "_mask = grism_trace_mask("
    "model.data, model.dq, dispersion_axis=_dispersion_axis(book)"
    ")",
    "model.data, model.err, model.dq = subtract_oneoverf(",
    '    model.data, model.err, model.dq, by="column", mask=_mask,',
    ")",
)

_TEMPLATE = '''"""Grism (WFSS) reduction runner generated by `noobfriend reduce gen`.

Reduce rate -> 2b per frame (assign_wcs, flat, WFSS master-sky, flag_outlier,
column 1/f), then build and subtract a per-(module, direction) sky-residual
template 2b -> 2bii. Run where the NooBox product locations open locally (e.g. on
the data server):
...
def render(recipe: Recipe) -> str:
    ...
    stage_2b = save.get("oneoverf", "2b")
    stage_2bii = save.get("template_bkg", "2bii")

    # The grism trace-mask / sky-template primitives are referenced in the
    # always-present two-pass scaffold (gated at runtime by DO_TEMPLATE), so
    # import them unconditionally; the detector-step imports (flag-outlier, column
    # 1/f) are added for whichever pass-1 steps are enabled.
    grism_names = [
    ...
    detector_names = {spec.custom for spec, _ in pass1_steps if spec.custom}
    if any(spec.name == "oneoverf" for spec, _ in pass1_steps):
        detector_names.add("subtract_oneoverf")
    reduction_import = (
        f"from noobfriend.reduction.grism import {', '.join(sorted(grism_names))}"
    )
    if detector_names:
        reduction_import += (
            "\nfrom noobfriend.reduction.detector import "
            f"{', '.join(sorted(detector_names))}"
        )
    ...
    for spec, _ in pass1_steps:
        ...
        elif spec.name == "oneoverf":
            pass1 += list(_ONEOVERF)
        else:
