...
@env_app.command(
    name="init",
    help="Initialize the .env file"
)
def init_env(
    start_stage: Annotated[str, typer.Option(
        '-s', '--start-stage',
    ...
    crds_path: Annotated[Path, typer.Option(
        '-c', '--crds-path',
        help="Path to the CRDS cache directory.",
        resolve_path=True,
        rich_help_panel="Setup"
    )] = None,
    data_root_path: Annotated[Path, typer.Option(
        '-d', '--data-root-path',
    ...
    show_content: Annotated[bool, typer.Option(
        '-v', '--show-content',
    ...
    console.print(f"[bold green]The .env file has been written to [yellow]{output_path / '.env'}[/yellow][/bold green]")
...
def stage_list_callback(value_list: list[str]) -> list[str]:
    ...
    if start_stage is None or start_stage[0] == '#':
        raise typer.BadParameter("The START_STAGE environment variable is not set in the .env file.")
...
@env_app.command(
    name='append',
    help="Append folder information to the .env file"
)
def append_env(
    stage_list: Annotated[list[str], typer.Argument(
        help="Stage to append to the .env file. Can be specified multiple times.",
        callback=stage_list_callback,
        metavar='LIST',
    )],
    auto_name: Annotated[bool, typer.Option(
        '-n', '--auto-name',
        help="Automatically name the paths based on the stage names.",
        is_flag=True,
    )] = False,
    append_at_end: Annotated[bool, typer.Option(
        '-a', '--append-at-end',
    ...
    for key in init_keys:
        ...
        if env_var is not None and env_var[0] != '#':
            ...
            console.print(f"[bold red]{key}[/bold red] is not set in the .env file. Please initialize the .env file first.")
    ...
    # Append mode
    if append_at_end:
        append_render_dict.pop(os.environ['START_STAGE'], None)  # Remove the start stage if it exists
        with open(".env", "a") as f:
            f.write(append_template.render(
                stage_folder_paths=append_render_dict
            ))
            console.print(f"[bold green]The .env file has been appended with the new stage paths.[/bold green]")
    # Replace mode
    else:
        rendered_content = init_template.render(
            **init_render_dict
        ...
        with open(".env", "w") as f:
            ...
            console.print(f"[bold green]The .env file has been updated with the new stage paths.[/bold green]")
...
def check_env(
    mkdir: Annotated[bool, typer.Option(
        '-m', '--mkdir',
    ...
    if not env_file.exists():
        console.print(f"[bold red]The .env file does not exist at [yellow]{env_file}[/yellow]. Please initialize the .env file first.[/bold red]")
        raise typer.Exit(code=1)
    ...
    console.print(f"[bold blue]Checking Paths in[/bold blue] [bold yellow]{env_file}...[/bold yellow]")

    paths_need_to_create = []
    
    for var_name, path in path_vars.items():
        console.print(f"[bold cyan]{var_name}[/bold cyan]:", end=' ')
        is_file = '.' in path.name and not path.name.startswith('.') # Check if the path is a file
        if is_file:
            path = path.parent
    ...
    if mkdir and paths_need_to_create:
        console.print(f"[bold yellow]Creating directories...[/bold yellow]")
